...
function showArticleIndex() {
	...
	var labelList = $("#article").children();//获取匹配元素集中每个元素的子元素。
	for ( var i=0; i<labelList.length; i++ ) {
		if ( $(labelList[i]).is("h1") ) {
			h2List = new Array();
			h1List.push({node: $(labelList[i]), id: i, children: h2List});//.is("h1")元素就加入列表
		}
	...
	$("aside #toc").empty(); //删除所有 <toc> 元素的内容：
	$("aside #toc").append(show(h1List));//在所有 <toc> 元素的末尾插入内容：
